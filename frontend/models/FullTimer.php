...
class FullTimer extends \yii\db\ActiveRecord
{
    ...
    public function getFullTimerWithPhoto()
    {
        return $this->find()->where('photo_file IS NOT NULL')->all();
    }

    public function getKarirData() {
        $karirSearchModel = new FullTimerKarirSearch();
        $karirQueryParams = ['FullTimerKarirSearch' => ['full_timer_id' => $this->id], 
                              'r'=>'full-timer-karir'];
        return $karirDataProvider = $karirSearchModel->search($karirQueryParams);    
    }
}
